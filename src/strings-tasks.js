...
function countVowels(str) {
  const letters = str.match(/[aeiouyAEIOUY]/gi);
...
function isPalindrome(str) {
  let newStr = '';
  newStr = str.toLowerCase().replace(/[^a-z]/gi, '');
...
function extractNameFromTemplate(value) {
  ...
  const template2 = '!';
  return value.replace(template, '').replace(template2, '');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (i) => cipherAlphabet[alphabet.indexOf(i)]);
}
